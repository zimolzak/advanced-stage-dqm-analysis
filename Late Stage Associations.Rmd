...
```{r init-completion-corr-plots, warning=FALSE}
reviewed_all %>% 
  filter(signal_present=="Yes", MOD_definite) %>% 
...
## INSERT ONE? Not as useful as MOD-only: Time from susp to dx, vs.\ MOD
...
reviewed_all %>% 
  ...
  filter(!is.na(  MOD_definite )) %>%    # join here
  ...
  geom_text(x = 1.25, y = 0.65, label="0.06", color="#444444", size=3) +
...
reviewed_all %>% 
  ...
  labs(y = "Workup initiation to completion (years)")
```
