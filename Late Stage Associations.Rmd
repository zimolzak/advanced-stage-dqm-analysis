...
## Initiation time vs. gender

```{r geis-temporal-assoc, message=FALSE, warning=TRUE}
temporal %>% 
  group_by(Sex) %>% 
  summarise(across(sig_to_init:init_to_complete, q123)) %>% 
  select(!contains(c("mean", "sd"))) %>% 
...
## Completion time vs. age (2 plots)
...
## Initiation time vs. completion time (probably too wacky)
