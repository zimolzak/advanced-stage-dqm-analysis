...
Output turned off---we do this better in a boxplot later.
...
chart_rev_confint <- bind_rows(
  ...
  mutate(Characteristic = case_when(
    Characteristic == "Ca considered No" ~ "Cancer not\nconsidered",
    
    Characteristic == "Ca signal Yes" ~ "Cancer signal\npresent",
    Characteristic == "Ever declined screening Yes" ~ 'Declined\nscreening',
...
## FIGURE or table to include - Time with custom breaks
...
## INSERT ONE: Time from susp to dx, vs.\ MOD

```{r assoc-most-detailed, warning=FALSE}
reviewed_all %>% 
  select(
  ...
  ggplot(aes(x=value, y = MOD_definite, color= MOD_definite)) +   # x=paste(site, cancer_type), y
  geom_jitter(alpha = 0.5) +
  ...
  labs(x="Years", y="MOD present")
```

I think the intervals just aren't collected a lot when MOD is false.
...
Exact quote of what was recommended to analyze:

> time from suspicion to diagnosis

What's 'suspicion'? Is it cancer signal? Or test ordered? Probably the latter.
So we will focus on `init_to_completion`.

```{r simpler-boxplot-for-r1, warning=FALSE}
reviewed_all %>% 
...
```{r simpler-boxplot-for-r1-ZOOM, warning=FALSE}
reviewed_all %>% 
  select(site, cancer_type, MOD_definite, init_to_completion) %>% 
  filter(
    !is.na(MOD_definite),
  ...
  labs(y = "Workup initiation to completion (days)")
```
...
temp_limit_to_mod <- reviewed_all %>%
  ...
  mutate(interval_color=case_when(
    interval_type == "init" ~ "cyan",
    interval_type == "completion" ~ "magenta"
  ))
  
LW  <-  1.3
  
temp_limit_to_mod %>% 
  ggplot(aes(y = value, x = patient_id, colour = paste(site, cancer_type))) +
  geom_linerange(
    aes(ymin = vmin, ymax=vmax),
    #color = temp_limit_to_mod$interval_color  # okay it's always magenta here,
    linewidth=LW
  ) +
  geom_linerange(aes(ymin = 0, ymax=vmin), color="lightgrey", linewidth=LW) +
  geom_point(aes(shape = factor(interval_type))) +
  coord_cartesian(ylim = c(-2, 750)) +
  scale_y_continuous(breaks = seq(0, 900, 180)) +
