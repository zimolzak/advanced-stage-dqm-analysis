...
temporal_gv %>% filter(site == "Geisinger", is.na(sig_to_init)) %>% select(cancer_type, signal_date, dx_eval_init_date, MOD_definite, signal_present) %>% View()
...
**Note to self!** Maybe (probably) I need to select only for those with red flag `= TRUE`.
