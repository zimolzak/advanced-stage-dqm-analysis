...
ggplot(reviewed_all %>% filter(site !="VA"), aes(y=Age)) +
  geom_boxplot() +
  facet_grid(cols=vars(cancer_type), rows=vars(site))
```
...
temp_boxplot_data %>% 
  ggplot(aes(y=Age, x  = paste(site, cancer_type), fill=site )) +   # fill=site
  geom_boxplot(staplewidth = 0.2, outlier.shape = 1, outlier.fill = "white") +
  geom_jitter(color = temp_boxplot_data$is_outlier ) +
  theme(
