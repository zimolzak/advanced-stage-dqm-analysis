...
- [x] `lung_geis_raw` (Geisinger chart review)
- [ ] `colon_geis_raw`
- [ ] `lung_va_raw`
- [ ] `colon_va_raw`
...
percentage_text <- function(n, d) {
  ns <- as.character(n)
  ds <- as.character(d)
  ps <- as.character(round(n / d * 100, 1))
  final_s <- paste0(
    ns, " / ", ds, " (", ps, "%)"
  )
  return(final_s)
...
rbind(
  ...
  lung_geis_raw %>% count(Sex) %>% mutate(d=sum(n), Fact="Sex") %>% rename(Val=Sex),
