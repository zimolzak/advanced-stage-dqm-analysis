...
reviewed_all %>% 
  ...
  filter(!is.na(MOD_definite)) %>% 
  unite(col = "Population", site:cancer_type, sep=" ") %>% 
  ggplot(aes(x= Years, y = Population, color= MOD_definite, group = paste(MOD_definite, Population))) +   # x=paste(site, cancer_type), y
  geom_jitter(alpha = 0.5) +
  geom_boxplot(outliers = FALSE, staplewidth = 0.5, varwidth = TRUE) +
