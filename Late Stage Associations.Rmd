...
```{r geis-table-ci}
rbind(
  reviewed_geis_lung %>% count(MOD_definite) %>% mutate(d=sum(n), Fact="MOD") %>% rename(Val=MOD_definite),
  reviewed_geis_lung %>% count(Sex) %>% mutate(d=sum(n), Fact="Sex") %>% rename(Val=Sex),
  reviewed_geis_lung %>% count(signal_present) %>% mutate(d=sum(n), Fact="Ca signal") %>% rename(Val=signal_present),
  reviewed_geis_lung %>% filter(!is.na(cancer_dx_considered)) %>% count(cancer_dx_considered) %>% mutate(d=sum(n), Fact="Ca considered") %>% rename(Val=cancer_dx_considered)
) %>% 
  mutate(
