...
reviewed_geis_colon <- colon_geis_raw %>% 
  ...
  mutate(
    ...
    MOD_present_main_outcome = as.factor(`Is there a missed opportunity (in general) in this case?`),
  ...
  relocate(
    ...
    signal_present, cancer_dx_considered, MOD_present_main_outcome,
...
reviewed_va_lung_colon <- lung_va_raw %>% 
  ...
  mutate(
    ...
    MOD_present_main_outcome = as.factor(DxMOD),
    signal_date = as.Date(RFInitialOccDateTime),
...
## Vars to consider tidying

- type of red flag: `RFInitialRF` in VA

## Which files/datasets to tidy
...
# Colon associations with advanced stage (tests, age plots)
...
# Lung associations with advanced stage (tests, age plots)
