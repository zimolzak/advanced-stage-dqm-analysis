...
- Chart reviews
    ...
    - [x] `lung_va_raw`
    - [x] `colon_va_raw`
    - [ ] ***VA date data!*** (re-export?)
    - [ ] Oh dear, what if "date of completion" varies by reviewer.
...
rbind(  # FIXME - should we use bind_rows() ?
  all_data %>% 
    group_by(site, cancer_type) %>% 
    count() %>%
...
```{r descriptives-continuous-strat, message=FALSE}
...
# Associations: chart review. WARNING, merged lung and colon
...
reviewed_all %>% 
  ...
  group_by(Sex) %>% 
  summarise(across(signal_to_init:init_to_completion, q123)) %>% 
...
# FIXME - something weird about how cor.test handles NAs.
...
t_all <- temporal_gv %>%
  ...
  select(cancer_type, site, Age, sig_to_init, init_to_complete)



cor.test(t_all$sig_to_init, t_all$init_to_complete)
```
```{r wacky-time-plot}
t_all %>% 
  ...
  geom_point(aes(colour = factor(interval_type)))
