...
chart_rev_confint <- bind_rows(
  ...
  bind_rows(va_population_ci)
```

```{r presentation-ci}
chart_rev_confint %>%
  select(!c(n, d, wilson.poi)) %>% 
...
## Table: quartiles of age and times
...
```{r fig-binning-custom-histo, warning=FALSE}
reviewed_all %>% 
...
## Plot and table: time with subset and capped
...
## Table: Advanced stage percentages vs race, sex
...
## Table: Age quartiles vs. advanced-stage status

This goes along with the density plots earlier. All $P$ values are by Wilcoxon.
...
## Table: stratified $P$ values (gender vs. time)
...
## Multiple correlations (continuous vs. continuous)
...
- [x] Get rid of all the `eval=FALSE`

- [x] Do something with `cor.test()` like the way we used `wilcox.test()` with a formula. See line 854 or so, above. Here is the function we will apply using `across()`:

`~ wilcox.test(. ~ Sex)$p.value`

...but probably need to use a `list()` or something, because extracting both $P$ and $\rho$.
...
```{r completion-age-plots, warning=FALSE}
reviewed_all %>% 
  filter(signal_present=="Yes", MOD_definite) %>% 
