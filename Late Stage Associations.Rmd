...
# FIXME - Associations: continuous vs. continuous
...
- [ ] Get rid of all the `eval=FALSE`

- [ ] Do something with `cor.test()` like the way we used `wilcox.test()` with a formula. See line 854 or so, above. Here is the function we will apply using `across()`:
...
```{r correlations-full-table}
pval_rho <- list(
  p_value = ~cor.test(., method = "spearman")$p.value,
  rho = ~cor.test(., method = "spearman")$estimate
)

cor.p <- function(vec1, vec2) {
...
reviewed_all %>% 
  ...
  filter(signal_present=="Yes", MOD_definite) %>% 
  summarise(
    across(where(~ signal_to_init + init_to_completion), pval_rho)
  # summarise(
  #   st12 = cor.test(~ signal_to_init + init_to_completion, method="spearman")$p.value
  )
```

```{r ia-test, eval=FALSE}
...
```{r completion-age-plots, eval=FALSE}
...
```{r init-completion-wacky-plots, eval=FALSE}
