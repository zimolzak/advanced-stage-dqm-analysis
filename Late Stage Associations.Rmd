...
chart_rev_confint <- bind_rows(
  ...
  reviewed_all_grp %>% 
    ...
    mutate(Fact = "    +S Pattern", Val=as.character(MOD_delay_pattern)),
  reviewed_all_grp %>% 
  ...
  filter(
    # Exclude 1 rows because it is redundant to say 25% true, 75% false.
    !(Fact == "MOD" & Val == FALSE)
  ) %>%
  unite(col=Characteristic, Fact, Val, sep=" ") %>%
  # mutate(Characteristic = case_when(  # Rename some cell contents for better readability.
  ...
  #   Characteristic == "Cancer signal present, given MOD No" ~ 'Missed screening, given MOD',
...
chart_rev_confint %>% 
  filter(!grepl("Race", Characteristic), !grepl("+S", Characteristic, fixed = TRUE), !grepl("-S", Characteristic)) %>%   # FIXME - can bring back later
  rename(Site = site) %>% 
...
t1t2_ordering <- tribble(
  ...
  "    +S Pattern Requesting", "    Delay initiating workup",
  "    +S Pattern Completing", "    Delay completing workup",
  "    +S Pattern Combined", "    Delay, combined",
  # "    +S Pattern Unspecified delay", "    Delay, unspecified",  # should not exist any more
  "    +S Pattern Timely", "    No delays",
  
  "Ca signal No","Cancer signal absent",
  "    -S Pattern Missed screening", "    Screening due but not performed",
  "    -S Pattern Screen-compliant", "    Up-to-date on screening per guidelines",
  "    -S Pattern Declined screening", "    Screening declined",
