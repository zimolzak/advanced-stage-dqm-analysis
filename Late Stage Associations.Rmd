...
## TABLE 1: summarizing all vars on whole population
...
Table will be rendered later (pending Geisinger chart review data).
...
# FIXME - left off here. Mutate Characteristic.
```
...
table_1_for_later %>% 
  ...
  left_join(
    ...
    select(Characteristic, `Geisinger Lung`, `Geisinger Colon`) %>%   # age, male
...
In panel one, `r cuts[1,2]` points are off scale, with a maximum of `r cuts[1,3]`.
In panel two, `r cuts[2,2]` points are off scale, with a maximum of `r cuts[2,3]`.
In panel three, `r cuts[3,2]` points are off scale, with a maximum of `r cuts[3,3]`.
