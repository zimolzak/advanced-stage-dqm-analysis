...
reviewed_all <- bind_rows(
  ...
  mutate(
    ...
    MOD_delay_pattern = as.factor(case_when(
      MOD_delay_completing=="Yes" & MOD_delay_requesting=="Yes" ~ "Both",
      MOD_delay_requesting=="Yes" ~ "Requesting",
      MOD_delay_completing=="Yes" ~ "Completing",
...
chart_rev_confint <- bind_rows(
  ...
  # FIXME - change how we throw out NAs?
  ...
  reviewed_all_grp %>% 
    ...
    mutate(cancer_dx_considered = case_when(
      is.na(cancer_dx_considered) ~ "Yes",  # benefit of the doubt
      .default = cancer_dx_considered
    )) %>% 
    count(cancer_dx_considered) %>%
    mutate(Fact = "Ca considered", Val=as.character(cancer_dx_considered)),
  ...
  reviewed_all_grp %>% 
    count(screening_declined) %>%
    # Note: no filter(!is.na())
    mutate(Fact = "Ever declined screening", Val=as.character(screening_declined)),
  reviewed_all_grp %>% 
    count(Race) %>%
    # Note: no filter(!is.na())
    mutate(Fact = "Race", Val=as.character(Race)),
  reviewed_all_grp %>% 
    filter(MOD_definite) %>% # There is a filter but it's different. Not to do with is.na(MOD)
    count(signal_present) %>% 
    mutate(Fact = "Cancer signal present, given MOD", Val=signal_present),
  va_population_ci
  ...
  #     Fact == "Race" |
  #     (Fact == "Cancer signal present, given MOD" & Val == "No")
  # ) %>% 
...
To do: LEFT OFF HERE Mar 19

- Cancer signal present
    - test ordered timely
    - test ordered delay: `DxDelayRFFUReq`
    - test not ordered
    - completion: `DxDelayRFFUComp`
- no signal but need screening
    - got screening
    - Missed screening
    - Declined screening
- MOD present
...
t1_df <- data.frame(
  Characteristic_short = c(
    ...
    "Other",
    "n",
    "Missed screening, given MOD"
  ),
  order = c(1, 7, 9, 8, 5, 3, 6, 4, 2, 2.5, 2.1, 2.15, 2.2, 2.3, 0, 5.9),
  Characteristic = c(
    "Age, y",
...
t1t2 <- reviewed_all %>% 
  ...
  right_join(t1_df) %>%  # Note: this has similar effect to filter. If row not mentioned, not included.
  arrange(order) %>% 
...
```{r table2}
# FIXME - Maybe should include denom later. See the other FIXME, above.
# For example table: Kesselheim et al. "Prevalence and Predictors of Generic Drug Skepticism Among Physicians."
# Maybe should change how we do the denoms too. See above. Not exclude NA?

t1t2 %>% 
  filter(order >= 3 ) %>% 
  mutate(Characteristic = case_when(
    Characteristic == "Cancer not considered" ~ "Cancer not considered, given signal",
    .default = Characteristic
  )) %>% 
  select(Characteristic, `VA Lung`, `VA Colon`, `Geisinger Lung`, `Geisinger Colon`) %>% 
  kable()
