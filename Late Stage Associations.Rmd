...
## Table summarizing all vars on whole population
...
## Temporality figures and table - INCLUDE one
...
temp_mod_sv <- reviewed_all %>% 
  ...
  rename(
    `Time 1 (signal to workup initiation)` = signal_to_init,
    `Time 2 (initiation to completion)` = init_to_completion,
    `Total (T1 + T2)` = signal_to_completion,
  ) %>% 
...
ggplot(aes(y = value, x = Population, fill=site)) +
  ...
  labs(title="Limited to MOD only", y = "Time, years") +
  theme(panel.grid.major.x = element_blank(), legend.position = "none") +
  scale_fill_hue(c=50)
```
...
## Race and sex vs.\ advanced - USEFUL TABLE
...
$\rho$ coefficients, with "*" meaning significant at $P < 0.05$.
...
```{r assoc-most-detailed, message=FALSE, warning=FALSE}
reviewed_all %>% 
  select(
