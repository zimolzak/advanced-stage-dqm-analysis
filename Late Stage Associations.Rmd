...
lung_geis_raw <- read_csv("final lung LS for Houston de-id Divvy 3-28-22.csv", col_types = coltype_spec) %>% 
  mutate(cancer_type = "Lung", site="Geisinger")
...
# Colon associations with advanced stage
...
# Lung associations with advanced stage
...
```{r geis-table-ci}
rbind(
  lung_geis_raw %>% count(MOD_definite) %>% mutate(d=sum(n), Fact="MOD") %>% rename(Val=MOD_definite),
  lung_geis_raw %>% count(Gender) %>% mutate(d=sum(n), Fact="Gender") %>% rename(Val=Gender),
  lung_geis_raw %>% count(signal_present) %>% mutate(d=sum(n), Fact="Ca signal") %>% rename(Val=signal_present),
  lung_geis_raw %>% filter(!is.na(cancer_dx_considered)) %>% count(cancer_dx_considered) %>% mutate(d=sum(n), Fact="Ca considered") %>% rename(Val=cancer_dx_considered)
) %>% 
  mutate(
  ...
  unite(wilson.poi, wilson.low, col="Proportion_Wilson_CI", sep = " (") %>% 
  unite(Proportion_Wilson_CI, wilson.upp, col = "Proportion_Wilson_CI", sep = " - ") %>% 
  mutate(Proportion_Wilson_CI = paste0(Proportion_Wilson_CI, ")")) %>% 
  relocate(Fact, Val, n, d) %>% 
  kable()
```

```{r geis-temporal}
temporal <- lung_geis_raw %>% 
  mutate(
...
wilc_s2i_gender <- wilcox.test(
  temporal %>% filter(Gender == "M") %>% pull(sig_to_init),
  temporal %>% filter(Gender == "F") %>% pull(sig_to_init)
)
wilc_s2i_gender

wilc_i2c_gender <- wilcox.test(
  temporal %>% filter(Gender == "M") %>% pull(init_to_complete),
  temporal %>% filter(Gender == "F") %>% pull(init_to_complete)
)
wilc_i2c_gender
...
cor.test(temporal$Age, temporal$init_to_complete, method = "spearman")  # stat signif
cor.test(temporal$sig_to_init, temporal$init_to_complete, method = "spearman")  # stat signif
