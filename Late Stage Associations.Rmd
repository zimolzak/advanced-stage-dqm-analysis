...
gmod <- lung_geis_raw %>% 
  count(MOD_definite) %>% 
  pivot_wider(names_from = MOD_definite, values_from = n, names_prefix = "MOD_") %>% 
  mutate(
    denom = MOD_FALSE + MOD_TRUE,
    wilson.poi = binconf(MOD_TRUE, denom)[,"PointEst"],
    wilson.low = binconf(MOD_TRUE, denom)[,"Lower"],
    wilson.upp = binconf(MOD_TRUE, denom)[,"Upper"]
  )
```
