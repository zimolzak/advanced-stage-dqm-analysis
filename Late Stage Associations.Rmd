...
exclusions <- all_data %>% 
  count(cancer_type, HasPCPAfterCutOff) %>% 
  filter(HasPCPAfterCutOff == 0) %>% 
  select(cancer_type, n)

ex_colon <- exclusions %>% filter(cancer_type=="Colon") %>% pull(n)
ex_lung  <- exclusions %>% filter(cancer_type=="Lung")  %>% pull(n)
```

## For text (1): INCLUDE

"We excluded `r ex_lung` lung cancer patients
and `r ex_colon` colorectal cancer patients from the cohort due to a lack of
primary care visits in the VA."
...
```{r exclude-yes-no, echo=TRUE}
DO_PCP_FILTERING <- TRUE

if (DO_PCP_FILTERING) {
  all_data <- all_data %>% 
    filter(HasPCPAfterCutOff == 1)
}
```
...
# Continue tidying manual review data
...
reviewed_geis_colon <- colon_geis_raw %>% 
  rename(
    ...
    MOD_delay_requesting = `Was there a delay (>7 days) in requesting follow-up for any of the cancer signals that the patient presented with?`,
    MOD_delay_completing = `Was there a delay (>60 days) in completing requested follow-up for the cancer signal?`
  ) %>% 
  mutate(
    ...
    signal_type = `What was the earliest cancer signal recorded?`,  # not as.factor
    signal_other_text = `Other(Specify):...23`,
    .keep = "unused"
  ) %>% 
  relocate(
    site, cancer_type, Age, Sex, Race, Location,
...
va_signal_decode <- tribble(
~cancer_type,  ~RFInitialRF,            ~Signal,
"Colon", 1,              "Iron Deficiency Anemia",
"Colon", 2,              "Bowel Obstruction",
"Colon", 3,              "Blood in Stool",
"Colon", 4,              "Abdominal Mass",
"Colon", 5,              "Rectal Mass",
"Colon", 6,              "Change in Stool Caliber",
"Colon", 7,              "Colonoscopy Finding",
"Colon", 8,              "Fecal Occult Blood Test Finding",
"Colon", 9,              "Flexible Sigmoidoscopy Finding",
"Colon", 10,           "Barium Enema Finding",
"Colon", 11,           "CT Imaging Finding",

"Colon", 12,           "Family Hx of Cancer in < 50 y/o",
"Colon", 13,           "Mass on exam",
"Colon", 14,           "Other",
...
"Lung", 3, "Chest pain",
"Lung", 4, "Hemoptysis",
"Lung", 5, "Recurrent bronchitis/pneumonia",
"Lung", 6, "Imaging in general",
"Lung", 7, "Paraneoplastic",
"Lung", 8, "Hoarseness lasting > 2 wk",
"Lung", 9, "Weight loss",
"Lung", 10, "Other"
...
reviewed_va_lung_colon <- lung_va_raw %>% 
  ...
  left_join(va_signal_decode, by = c("RFInitialRF", "cancer_type")) %>%
  mutate(
    siginit = case_when(
      siginit < -40000 ~ NA,
      .default = siginit
    ...
    signal_type = Signal  # not as.factor
  ) %>% 
  rename(
    screening_declined = PtDecline,
    MOD_delay_requesting = DxDelayRFFUReq,
    MOD_delay_completing = DxDelayRFFUComp
  ) %>% 
  relocate(
    site, cancer_type,  # Age, Sex, Race, Location,
    signal_present, cancer_dx_considered, MOD_definite,
...
reviewed_all <- bind_rows(
  ...
  select(
    ...
    signal_type, signal_other_text,
    MOD_delay_requesting, MOD_delay_completing
  ) %>% 
  mutate(
    ...
    MOD_delay_pattern = as.factor(case_when(
      signal_present=="Yes" & MOD_delay_completing=="Yes" & MOD_delay_requesting=="Yes" ~ "Combined",
      signal_present=="Yes" & MOD_delay_requesting=="Yes" ~ "Requesting",
      signal_present=="Yes" & MOD_delay_completing=="Yes" ~ "Completing",
      # All combos Yes-any, any-Yes, Yes-Yes are now covered.
      
      signal_present=="Yes" & (!MOD_definite) ~ "Timely",
      MOD_definite & signal_present=="Yes" ~ "Combined",  # IMPORTANT: This now adds 2 patterns together
      # All signal=Yes are now covered.
      
      (signal_present=="No" | signal_present=="Unsure") & MOD_definite ~ "Missed screening",
      (signal_present=="No" | signal_present=="Unsure")  & (!MOD_definite) & screening_declined=="Yes" ~ "Declined screening",
      (signal_present=="No" | signal_present=="Unsure")  & (!MOD_definite) ~ "Screen-compliant"
    ))
  )
```

## Confirm some things about cancer signal symptom coding

Just confirming that this source variable only goes up to 12. Not 14 like in the Word doc:

```{r confirm-crc}
reviewed_va_lung_colon %>% 
  filter(cancer_type == "Colon") %>%
  count(RFInitialRF, signal_type) %>% 
  kable()
```


Just confirming for lung. This seems to go up to 8.
...
```{r cf-geisinger}
reviewed_geis_lung %>% 
  mutate(signal_type = substr(signal_type, 1, 40)) %>% 
  count(signal_type) %>% 
  kable()
```

```{r DELETE-chart-rev-bits, eval=FALSE, include=FALSE}
...
```{r DF-table-1-for-later}
# Basically sex, race, age, stage, mainly on VA data.
...
## *Static* Text for mini abstract: INCLUDE

proportion of advanced-stage lung cancer was 45.9% at the VA and
58.3% at Geisinger Health System, with frequent missed opportunities (58.0% and 77.8%,
respectively). The proportion of advanced-stage colorectal
cancer was 33.2% at the VA medical center and 36.2% at Geisinger Health System, and
most patients had missed opportunities (66.3% and 69.7%, respectively)
...
```{r DF-chart-rev-confint-IMPORTANT}

# The chart review proportions that become Table 2.
...
# Big important data frame coming.
# One of the last times you see actual numeric values rather than character.

reviewed_all_grp <- reviewed_all %>%
  group_by(site, cancer_type) %>% 
  mutate(
    signal_present = case_when(
      signal_present == "Unsure" ~ "No",  # benefit of the doubt
      .default = signal_present
    ),
    cancer_dx_considered = case_when(
      cancer_dx_considered == "Unsure" ~ "Yes",  # benefit of the doubt
      .default = cancer_dx_considered
    ),
    MOD_delay_requesting = case_when(
      MOD_delay_requesting == "Unsure" ~ "No",
      .default = MOD_delay_requesting
    ),
    MOD_delay_completing = case_when(
      MOD_delay_completing == "Unsure" ~ "No",
      .default = MOD_delay_completing
    ),
    MOD_definite = case_when(
      MOD_delay_requesting == "Yes" ~ TRUE,  # fix glitch in review data, only 1 row.
      .default = MOD_definite
    )
  ) %>% 
  replace_na(list(MOD_delay_requesting = "No", MOD_delay_completing = "No"))
...
chart_rev_confint <- bind_rows(
  ...
  reviewed_all_grp %>% 
    count(Sex) %>% 
    filter(!is.na(Sex)) %>%
    mutate(Fact = "Sex", Val=as.character(Sex)),
  reviewed_all_grp %>% 
    count(Race) %>%
    # Note: no filter(!is.na())
    mutate(Fact = "Race", Val=as.character(Race)),

  # Table 2
  
  reviewed_all_grp %>% 
    count(signal_present) %>% 
    filter(!is.na(signal_present)) %>%
    mutate(Fact = "Ca signal", Val=as.character(signal_present)),
  reviewed_all_grp %>% 
    filter(!is.na(signal_present)) %>% 
    count(MOD_delay_pattern) %>%
    mutate(Fact = "Pattern", Val=as.character(MOD_delay_pattern)),
  reviewed_all_grp %>% 
    filter(!is.na(MOD_definite), !is.na(signal_present)) %>%
    count(MOD_definite) %>% 
    mutate(Fact = "MOD", Val=as.character(MOD_definite)),
  va_population_ci
) %>%  # End of very big bind_rows().
  
  # Calculate % and CI
  
  group_by(site, cancer_type, Fact) %>% 
  ...
  filter(
    # Exclude 1 row because it is redundant to say 25% true, 75% false.
    !(Fact == "MOD" & Val == FALSE)
  ) %>%
  unite(col=Characteristic, Fact, Val, sep=" ") %>%
  arrange(desc(site), desc(cancer_type))
...
```{r DF-review-ci-concise}
denoms_temporary <- chart_rev_confint %>% 
  filter(Characteristic == "Ca signal Yes") %>% 
  select(site, cancer_type, d) %>% 
  mutate(d = as.character(d)) %>% 
  pivot_wider(names_from = c(site, cancer_type), values_from = d, names_sep = " ") %>% 
  mutate(Characteristic_short = "denominator")
...
Why do the denominators in that detailed table vary a lot?

```{r summary-NA}
reviewed_all %>% 
  select(MOD_definite, Sex, signal_present) %>% 
  summary()
```

Answer: because we do `filter(!is.na(...))` on certain vars to get the denominators.
But how can there be `NA` values for `MOD_definite` and `signal_present`? Shouldn't
reviewers always log those? Let's look at the times when MOD is listed as `NA`
(here in VA data only):

```{r eda}
reviewed_va_lung_colon %>% 
  filter(is.na(DxMOD)) %>% 
  select(site, cancer_type, RegConfCancerType, RegConfDxDate, StageNewDx, StageDataRecorded, DxMOD) %>% 
  kable()
```

Answer: When the dQM "thinks" there is a real lung cancer diagnosis, on a certain
date, and it's a first-time diagnosis, and it's a certain stage, *sometimes* the
algorithm is wrong. Reviewers didn't formally look for MOD in those cases. I don't
think there is one "right" way to treat the arithmetic here. Which question do you
want to answer?

a. Of all 100 charts that the dQM "thinks" are incident advanced colorectal, how many have MOD? (kind of like PPV)
b. Of all 95 charts with *actual* incident advanced colorectal, how many have MOD? (more like MOD rate)

Decided: I'm taking option b. That keeps the analysis as-is, with the denominators that vary a bit.
That is also most consistent across VA vs.\ Geisinger data.
...
chart_rev_confint %>% 
  filter(!grepl("Race", Characteristic), !grepl("No", Characteristic), !grepl("Sex F", Characteristic)) %>%   # FIXME - can bring back later
  rename(Site = site) %>% 
  ...
  #   labels = c("Ca. not\nconsid.", 'Cancer\nsignal', 'Declined\nscreen', 'Male', 'Screening\nMOD', 'MOD')  # FIXME - fragile
  # ) +
  ylim(0,1) +
...
```{r DF-t1t2, message=FALSE}
t1t2_ordering <- tribble(
  ~Characteristic_short, ~Characteristic,
  "n","n",
  "Age", "Age, y",
  "Sex M","Male",
  "White","White",
  ...
  "Ca signal Yes","Cancer signal present *",
  "Pattern Requesting", "    Delay initiating workup **",
  "Pattern Completing", "    Delay completing workup **",
  "Pattern Combined", "    Delay, combined **",
  # "    +S Pattern Unspecified delay", "    Delay, unspecified",  # should not exist any more
  "Pattern Timely", "    Workup initiated and completed with no delay",
  
  "Ca signal No","Cancer signal absent *",
  "Pattern Missed screening", "    Screening due but not performed **",
  "Pattern Screen-compliant", "    Up-to-date on screening per guidelines",
  "Pattern Declined screening", "    Screening declined",
  
  "MOD TRUE","MOD present",
  "signal_to_init", "Cancer signal to workup initiation, d",
  "init_to_completion", "Workup initiation to completion, d",
  "signal_to_completion", "Cancer signal to workup completion, d",
) %>% 
  mutate(k = 1, order = row_number(k)) %>% 
  select(order, Characteristic_short, Characteristic)
...
t1t2 <- reviewed_all %>% 
  ...
  bind_rows(denoms_temporary) %>% 
  right_join(t1t2_ordering) %>%  # Note: this has similar effect to filter. If row not mentioned, not included.
  arrange(order) %>% 
  select(order, Characteristic, `VA Lung`, `VA Colon`, `Geisinger Lung`, `Geisinger Colon`)

# Just print the Table 1 as a kable().
table_1_for_later %>%   # mainly VA stuff
  mutate(Characteristic = case_when(
  ...
  left_join(
    t1t2 %>% 
    filter(order < 9, Characteristic != "Advanced stage") %>% 
  ...
  rename(`VA Colorectal` = `VA Colon`, `Geisinger Colorectal` = `Geisinger Colon`) %>% 
  kable()
```

If `DO_PCP_FILTERING` is off, we expect the first row ($n$ for VA Lung and VA Colon,
respectively) to equal 37949 and 14753. With an advanced stage rate (last row) of:

- 17490 / 37949 (46.1%)
- 4913 / 14753 (33.3%)

**Current value** of `DO_PCP_FILTERING` is: `r DO_PCP_FILTERING`.

## To do after copy/paste

- [ ] Insert blank row reading "Race"
- [ ] Indent the race factor value rows
- [ ] put the $n$ in parentheses in the header row
- [ ] lose the denom in last row
- [ ] Put asterisks in the sex and race cells for Geisinger.
...
```{r table2}
# FIXME - Maybe should print out the numer & denom eventually, for clarity. See the other FIXME, above.
# For an example table: Kesselheim et al. "Prevalence and Predictors of Generic Drug Skepticism Among Physicians."

t1t2 %>% 
  filter(order >= 9 ) %>% 
  select(Characteristic, `VA Lung`, `VA Colon`, `Geisinger Lung`, `Geisinger Colon`) %>% 
  replace_na(list(`VA Lung` = "0", `VA Colon` = "0", `Geisinger Lung` = "0" )) %>% 
  rename(`VA Colorectal` = `VA Colon`, `Geisinger Colorectal` = `Geisinger Colon`) %>% 
  kable()
```
...
- [ ] Indent the appropriate rows.
- [ ] Times 10 point
- [ ] Bold
- [ ] resize columns, working right to left
- [ ] lose "denominator"




# A few plots, mostly turned off
...
```{r distribution-sx}
distribution_of_symptoms <- reviewed_all %>% 
  filter(signal_present == "Yes") %>% 
  mutate(
    signal_type = case_when(  # Compare to lines 233, "decode".
      signal_type == "imaging in general (should already be covered in dropdown list)" ~ "Imaging in general",
      signal_type == "Cough (Worsening persistent cough/bronchitis or new description of chronic cough lasting > 8 weeks.)" ~ "Cough",
      signal_type == "weight loss (Provider-acknowledged unexplained weight loss or other unexplained weight loss > 10 lbs in addition to respiratory symptoms)" ~ "Weight loss",
      signal_type == "recurrent bronchitis/pneumonia (an index infection, followed by 2nd pneumonia at least 1 month after index, no more than 1 year after index.)" ~ "Recurrent bronchitis/pneumonia",
      signal_type == "CT Imaging Finding" ~ "CT Imaging Finding",  # save this from sentence case
      is.na(signal_type) ~ "Other",  # Fold the rare NA values into Other.
      str_like(signal_other_text, "%pain%") ~ "Other",  # Pain, anemia, wt loss, habits now all merged into Other.
      str_like(signal_other_text, "%ida%") ~ "Iron deficiency anemia",  # merge
      str_like(signal_other_text, "%dec hb%") ~ "Other",
      str_like(signal_other_text, "%anemia%") ~ "Other",
      str_like(signal_other_text, "%blood%") ~ "Blood in stool",  # merge
      str_like(signal_other_text, "%discomf%") ~ "Other",
      str_like(signal_other_text, "%loss%") ~ "Other",
      str_like(signal_other_text, "%diarrh%") ~ "Other",
      str_like(signal_other_text, "%cologuard%") ~ "Fecal occult blood test finding",  # merge
      str_like(signal_other_text, "%habit%") ~ "Other",
      str_like(signal_other_text, "%constip%") ~ "Other",
      str_like(signal_other_text, "%leakage%") ~ "Other",
      str_like(signal_other_text, "%polyp%") ~ "Colonoscopy finding",  # merge
      .default = str_to_sentence(signal_type)
    )
  ) %>% 
  count(site, cancer_type, signal_type) %>% 
  arrange(desc(cancer_type), desc(site)) %>% 
  pivot_wider(names_from = site, values_from = n, values_fill = 0) %>% 
  mutate(tot = case_when(
    str_like(signal_type, "Other%") ~ 0,
    .default = VA + Geisinger
  )) %>% 
  arrange(desc(cancer_type), desc(tot), desc(VA), signal_type) %>% 
  select(cancer_type, signal_type, VA, Geisinger) %>% 
  rename(`Cancer type` = cancer_type, `Cancer signal` = signal_type)

distribution_of_symptoms %>% 
  kable()
```

## To do after copy/paste

- [ ] put a line under colon
- [ ] change to colorectal
- [ ] write out abbrs


## Side analysis: free text list

Someone will clearly want to know what the Geisinger "Other" signals represent.

```{r geis-other}
TEXT_CUTOFF = 100

reviewed_all %>% 
  filter(signal_type=="Other") %>% 
  mutate(
    dots = case_when(
      nchar(signal_other_text) > TEXT_CUTOFF ~ "...",
      .default = ""
    ),
    signal_other_text = paste(substr(signal_other_text, 1, TEXT_CUTOFF), dots)
  ) %>% 
  mutate(coded = case_when(
    str_like(signal_other_text, "%pain%") ~ "Other, pain",
    str_like(signal_other_text, "%ida%") ~ "Iron deficiency anemia",
    str_like(signal_other_text, "%dec hb%") ~ "Other, anemia",
    str_like(signal_other_text, "%anemia%") ~ "Other, anemia",
    str_like(signal_other_text, "%blood%") ~ "Blood in stool",
    str_like(signal_other_text, "%discomf%") ~ "Other, pain",
    str_like(signal_other_text, "%loss%") ~ "Other, weight loss",
    str_like(signal_other_text, "%diarrh%") ~ "Other, habits",
    str_like(signal_other_text, "%cologuard%") ~ "Fecal occult blood test finding",
    str_like(signal_other_text, "%habit%") ~ "Other, habits",
    str_like(signal_other_text, "%constip%") ~ "Other, habits",
    str_like(signal_other_text, "%leakage%") ~ "Other, habits",
    str_like(signal_other_text, "%polyp%") ~ "Colonoscopy finding",
    .default = "Other"
  )) %>% 
  select(cancer_type, coded, signal_other_text) %>% 
  arrange(coded) %>% 
  kable()
```
...
Y_MAX <- 8  # 8 almost okay but boring. 6 slightly better.

fig1 <- temp_mod_sv %>% 
  mutate(Population = Population_n) %>% 
ggplot(aes(y = value, x = Population, fill=site)) +
  geom_boxplot(staplewidth = 0.15, outlier.shape = 1) +
...
ggsave("fig1.eps", width = 6.5, height=7)
```

## Table of median months

```{r mod-severity-table-for-abstract, message=FALSE}
temp_mod_sv %>% 
  group_by(Population_s, name) %>% 
  summarise(median_yrs = median(value)) %>% 
  mutate(median_mo = round(median_yrs * 12, 1)) %>% 
  select(!median_yrs) %>% 
  arrange(name, desc(Population_s)) %>% 
  pivot_wider(values_from = median_mo, names_from = Population_s) %>% 
  kable()
```


## For text (2): About off-scale points. INCLUDE
...
## Side analysis: $n$ points

In case someone asks, "Why 200-something points per panel?":

```{r mods-per-stratum, message=FALSE}
temp_mod_sv %>%
  count(site, cancer_type, MOD_definite, name) %>%
  group_by(site, cancer_type, MOD_definite) %>%
  summarise(max = max(n)) %>% 
  ungroup() %>% 
  mutate(total = sum(max)) %>% 
  kable()
```
...
```{r sensitivity-boxplot}
reviewed_all %>% 
  filter(MOD_delay_requesting != "Unsure", MOD_delay_completing != "Unsure") %>% 
  ggplot(aes(x=signal_to_init, paste(site, cancer_type, MOD_delay_requesting), color=MOD_delay_requesting )) +
  geom_boxplot() +
  coord_cartesian(xlim = c(0,180))


# site, cancer_type, MOD_definite, MOD_delay_requesting, MOD_delay_completing
```
...
sens_table <- reviewed_all %>% 
  select(site, cancer_type, MOD_definite, signal_to_init, signal_to_init, init_to_completion) %>% 
  filter(MOD_definite) %>% 
  mutate(
    # 60 colon 30 lung.
    init7 = (signal_to_init > ITIMES[1]),
    init14 = (signal_to_init > ITIMES[2]),
    comp3060 = case_when(
      cancer_type == "Colon" ~ (init_to_completion > CTIMES[1]),
      cancer_type == "Lung" ~ (init_to_completion > LTIMES[1]),
    ),
    comp6090 = case_when(
      cancer_type == "Colon" ~ (init_to_completion > CTIMES[2]),
      cancer_type == "Lung" ~ (init_to_completion > LTIMES[2]),
    ),
    type = case_when(
      init7 != init14 ~ '7-14',
      comp6090 != comp3060 & cancer_type == "Colon" ~ 'col60-90',
      comp6090 != comp3060 & cancer_type == "Lung" ~ 'lun30-60',
    ),
    reclassified = (init7 != init14) | (comp6090 != comp3060)
  ) %>% 
  count(
    MOD_definite,
     type, reclassified, cancer_type
  ) %>% 
  arrange(cancer_type) %>% 
  group_by(cancer_type) %>% 
  mutate(
    tot_ca = sum(n),
    pct_ca = round(n / tot_ca * 100, 1)
  ) %>% 
  ungroup() %>% 
  mutate(
    tot = sum(n),
  ...
  mutate(
    n_grp = sum(n),
    pct_grp = round(n_grp / tot * 100, 1)
  ) %>% 
  arrange(type) %>% 
  select(!MOD_definite & !reclassified)

sens_table %>% 
  kable()

## prep vars for printing

type_a_list <- sens_table %>% 
  filter(type=="7-14") %>% 
  pull(pct_grp)

init_diff <- type_a_list[1]

col_diff <- sens_table %>% 
  filter(type=="col60-90") %>% 
  pull(pct_ca)

lung_diff <- sens_table %>% 
  filter(type=="lun30-60") %>% 
  pull(pct_ca)
```

## For text (3): INCLUDE

In sensitivity analysis, `r init_diff` percent of all missed opportunities would be excluded if the time to initiate workup was changed from `r ITIMES[1]` to `r ITIMES[2]` days.
If time to complete colorectal cancer workup was changed from `r CTIMES[1]` to `r CTIMES[2]` days, `r col_diff` percent of colorectal missed opportunities would be excluded.
If time to complete lung cancer workup was changed from `r LTIMES[1]` to `r LTIMES[2]` days, `r lung_diff` percent of lung missed opportunities would be excluded.




# TABLE 4: Race and sex $\sim$ advanced
...
The following table explains what's going on in detail with statistical
hypothesis tests of race and sex (as opposed to the short version in text).
...
- [ ] insert leader rows reading 'race' and 'sex'
- [ ] indent factor levels
- [ ] delete column 1
- [ ] put $P$ values in leader rows only
...
reviewed_all %>% 
  ...
  filter(MOD_definite) %>% 
  count(signal_present) %>% 
  mutate(total_MODs = sum(n)) %>% 
  filter(signal_present == "No")
```
