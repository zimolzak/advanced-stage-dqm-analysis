...
# FIXME - maybe do it as group_by in future.
```
...
reviewed_all %>% 
  group_by(site, cancer_type) %>% 
  summarise(across(c(Age, signal_to_init, init_to_completion, signal_to_completion), q123)) %>% 
  pivot_longer(
...
# Associations: chart review. FIXME: need to stratify lung and colon

Left off here. *leftoff*

## Associations needed to hypoth-test

- $t_{si}$ gender $\times \{l, c\} \times \{v, g\}$, with quartiles
- $t_{ic}$ gender $\times \{l, c\} \times \{v, g\}$, with quartiles
- $t_{si}$ Age $\times \{l, c\} \times \{v, g\}$
- $t_{ic}$ Age $\times \{l, c\} \times \{v, g\}$
- $t_{si} t_{ic} \times \{l, c\} \times \{v, g\}$
...
tribble(
  ...
  "Sex",  "t_si", "Lung", "Geisinger", 0.07,
  "sex", "t_si", "Colon", "Geisinger",  0.22,
) %>% 
  kable()
...
wilc_s2i_gender <- wilcox.test(
  reviewed_all %>% filter(Sex == "M") %>% pull(signal_to_init),
  reviewed_all %>% filter(Sex == "F") %>% pull(signal_to_init)
)
# FIXME - This stat test merges lung & colon.
# stratify
# probably filter NA to be safe
# Plots are okay, though!
wilc_s2i_gender
...
wilc_i2c_gender <- wilcox.test(
  reviewed_all %>% filter(Sex == "M") %>% pull(init_to_completion),
  reviewed_all %>% filter(Sex == "F") %>% pull(init_to_completion)
) # FIXME
# stratify
# filter to be safe
wilc_i2c_gender
...
```{r time-gender-table}
pval_row <- tribble(
  ~Sex, ~signal_to_init, ~init_to_completion,
...
reviewed_all %>% 
  ...
  summarise(across(signal_to_init:init_to_completion, q123), n=n()) %>% 
